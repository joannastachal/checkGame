...
ws.onmessage = function (evt) {
    ...
    if (data.substr(13, 1) === "2" && evt.data.startsWith('PLAYER_MOVED')) {
    ...
    if (data.startsWith('PLAYER_REGISTERED')) {
        ws.send("MOVE 3");
    ...
    if (evt.data.startsWith('PLAYER_MOVED')) {
        ...
        if (checkForWin(0, originalGameField) != undefined) {
            result = checkForWin(0, originalGameField).xAxis;
            ws.send("MOVE " + result);
            console.log("Win: ", result)
            return
        } else if (checkForWin(1, originalGameField) != undefined) {
            var result2 = checkForWin(1, originalGameField).xAxis;
